from gordon.services.validator.acquisition.acquistion_about_yaml import AboutYaml, AboutYamlException
...
from gordon.services.validator.acquisition import acquistion_constants as constants

logger = get_task_logger(__name__)
'''
The files in this directory serve as an example of how you would add another field to the about.yaml spec and validate those
fields. This can be used in your acquisitions where it makes sense to add additional fields
'''


class AcquistionFileValidator:
    ...
    def check_executor(self):
        ...
        try:
            ...
            """
            If you want to change or modify the default about.yaml specification for your organization then pay close attention
            to the the identifiers passed in check_message_constructor(). Say you want to add a new field to your about.yaml
            file -> slack_id: channel123.
            This would make the about.yaml spec as follows:
            ***********************************************
            version: 1
            ...
            ***********************************************
            Add slack_id in acquisition_schema.json so it's included in schema checks. Also ensure to add a new
            method something on the lines of is_valid_slack() in acquisition_about_yaml.py and call this method in the conditional statements
            in the blocks defined below. This is to include slack_id checks during the service run and is reflected in the end status check update
            Note: call the methods is_valid_slack() and check_message_constructor() for both the default and ref branch files
            The if-else decision tree below covers all possible scenarios of where and how about.yaml file can change and needs validation.
            It looks at the about.yaml files in the two branches involved in a pull request.
            """
            ...
            if not main_file and not ref_file:
                response = git_service.update_check_run(
                    ...
                    conclusion=constants.CHECKRUN_CONCLUSIONS["failure"])
            ...
            # block 2:- Decision tree to check validity of file if found in both main and ref branches
                ...
                if main_file.is_valid and ref_file.is_valid:
                    ...
                    self.check_message_constructor(
                        "slack_id", main_file.is_valid_slack, {
                            'slack_id': main_file.data["slack_id"]
                        })
                    ...
                    self.check_message_constructor(
                        "slack_id", ref_file.is_valid_slack, {
                            'slack_id': ref_file.data["slack_id"]
                        })
                    ...
                    if main_file.is_valid_slack and main_file.is_valid_pagerduty and main_file.is_valid_jira:
                        if not ref_file.is_valid_pagerduty or not ref_file.is_valid_jira or not ref_file.is_valid_slack:
                            response = git_service.update_check_run(
                                self.payload,
                            ...
                            response = git_service.update_check_run(
                                self.payload,
                                "All checks Passed",
                                conclusion=constants.CHECKRUN_CONCLUSIONS["success"])

                    # If main branch doesn't have valid data then check if ref is correcting it in the commit
                    elif ref_file.is_valid_slack and ref_file.is_valid_pagerduty and ref_file. is_valid_jira:
                        response = git_service.update_check_run(
                ...
                # block 2.2:- If main branch schema is valid and ref branch schema is invalid then fail the check as the
                # merge will corrupt the existing valid about.yaml in the main branch
                ...
                # valid. Else fail check since it doesn't fix an existing issue in main branch
                    ...
                    if not ref_file.is_valid_pagerduty or not ref_file.is_valid_slack or not ref_file.is_valid_jira:
                        response = git_service.update_check_run(
                            self.payload,
                ...
                # block 2.4:- If main and ref branches do not a valid schema then immediately fail the PR check
            ...
            # check if individual info in is valid or not. If valid then pass, else fail the check
            ...
            # If schema is valid, then check file info. If valid the pass check, else fail check
                ...
                if ref_file.is_valid:
                    ...
                    if not ref_file.is_valid_slack or not ref_file.is_valid_pagerduty or not ref_file.is_valid_jira:
                        response = git_service.update_check_run(
                            self.payload,
                            f"PR has failed about.yaml validation checks. Latest commit {self.ref_sha} invalidates the about.yaml in main branch. "
    ...
    # Method to construct the message seen on the status check of a Pull request after completion
        ...
        try:
            ...
            if identifier == "schema_message":
                if not status:
                    ...
                    with open(constants.CURRENT_DIR + f"/valid_scehmas/acquisition.txt") as file:
                        message += file.read()
                ...
                if not status:
                    message += f"<b>Jira Check:</b> <i>FAILED</i>. <b>Description:</b> {data['jira_id']} is not a " \
                               f"valid Jira project ID. Please provide an active and valid Jira project ID"

            elif identifier == "Slack_id":
                if not status:
                    message += f"<b>Slack Check:</b> <i>FAILED</i>. <b>Description:</b> {data['slack_id']} is not a " \
                               f"valid Slack channel ID. Please provide an active and valid Slack channel ID"
